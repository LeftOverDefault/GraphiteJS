...
function surfaceTranslate(surface, position = { x: 0, y: 0 }) {
    surface.translation.x = position.x;
    surface.translation.y = position.y;
}
...
function rectTranslate(rect, position = { x: 0, y: 0 }) {
    rect.x = position.x;
    rect.y = position.y;
}
...
class Rect {
    ...
    collidePoint(point = { x, y }) {
        return (
            this.x <= point.x <= this.x + this.width &&
            this.y <= point.y <= this.y + this.height
        );
    }
...
/**
 * Network object for communicating with a server.
 * @constructor
